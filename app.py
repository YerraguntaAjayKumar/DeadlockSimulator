...
if mode == "🏦 Banker's Algorithm (Resource Control)":
    ...
    # Process and Resource controls - Simplified
    ...
    with cfg_col1:
        ...
        pc1.markdown(f"### {st.session_state.b_n}")
        if pc2.button("➕", key="bp_add"): 
            st.session_state.b_n = min(st.session_state.b_n + 1, 10)
            st.rerun()
        if pc3.button("➖", key="bp_rm"):  
    ...
    with cfg_col2:
        ...
        rc1.markdown(f"### {st.session_state.b_m}")
        if rc2.button("➕", key="br_add"): 
            st.session_state.b_m = min(st.session_state.b_m + 1, 8)
            st.rerun()
        if rc3.button("➖", key="br_rm"):  
    ...
    # Three-column layout for matrices
    mat_col1, mat_col2, mat_col3 = st.columns(3)
    
    with mat_col1:
        # ALLOCATION MATRIX
        st.markdown("**📦 Allocation**")
        st.caption("Currently held")
        
        for i in range(n_p):
            st.markdown(f"**P{i}**")
            alloc_cols = st.columns(n_r)
            for j in range(n_r):
                with alloc_cols[j]:
                    v = st.number_input(f"R{j}", min_value=0, max_value=99, 
                                       value=st.session_state.b_allocation[i][j],
                                       key=f"alloc_{i}_{j}", label_visibility="collapsed")
                    st.session_state.b_allocation[i][j] = v
    
    with mat_col2:
        # MAX MATRIX
        st.markdown("**📋 Max Need**")
        st.caption("Maximum claim")
        
        for i in range(n_p):
            st.markdown(f"**P{i}**")
            max_cols = st.columns(n_r)
            for j in range(n_r):
                with max_cols[j]:
                    v = st.number_input(f"R{j}", min_value=0, max_value=99, 
                                       value=st.session_state.b_max[i][j],
                                       key=f"max_{i}_{j}", label_visibility="collapsed")
                    st.session_state.b_max[i][j] = v
    
    with mat_col3:
        # AVAILABLE RESOURCES
        st.markdown("**💎 Available**")
        st.caption("Free pool")
        
        avail_cols = st.columns(n_r)
        for j in range(n_r):
            with avail_cols[j]:
                st.markdown(f"**R{j}**")
                v = st.number_input("", min_value=0, max_value=99, 
                                   value=st.session_state.b_available[j],
                                   key=f"avail_{j}", label_visibility="collapsed")
                st.session_state.b_available[j] = v
    ...
    with btn_col1:
        if st.button("▶️ Check Safety", type="primary", use_container_width=True):
            ...
            if is_safe:
                st.success(f"✅ SAFE - Sequence: {' → '.join([f'P{p}' for p in safe_seq])}")
            else:
                st.error("⚠️ UNSAFE - Deadlock possible!")
    
    with btn_col2:
        if st.button("🔄 Reset", use_container_width=True):
            st.session_state.b_allocation = [[0 for _ in range(n_r)] for _ in range(n_p)]
            st.session_state.b_available = [5 for _ in range(n_r)]
            st.session_state.b_safe_sequence = []
            update_need_matrix()
